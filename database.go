...
func data_kategori(){
	...
	kategori[0].index = 0
	kategori[0].nama = "minuman"
	
	kategori[1].index = 1
	kategori[1].nama = "makanan"
...
func show_database(){
	for i:=0; i < 18; i++ {
		fmt.Printf("%-11d %-35s %-16s %-15d %d \n",gudang[i].index, gudang[i].nama, gudang[i].kategori.nama, gudang[i].harga, gudang[i].stok)
	}
	fmt.Println()
}